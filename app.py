...
with gr.Blocks() as app:
    gr.Markdown("## 📊 Student Performance Analysis System")

    with gr.Row():
        with gr.Column(scale=1):
            file = gr.File(label="📂 Upload Student CSV")
            analyze_btn = gr.Button("🔍 Analyze Dataset")
            dataset = gr.Dataframe(label="📄 Dataset Preview")
            eda_box = gr.Textbox(label="📌 EDA Summary", lines=6)
